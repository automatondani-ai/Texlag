    ...
    <div className="quote-result">
      <div className="quote-result__header">
        ...
        <div className="quote-result__meta">
          {quote.totalMiles} mi · {quote.driverMode} driver
          {quote.jurisdiction ? ` · ${quote.jurisdiction}` : ''}
        </div>
      </div>
      <div className="quote-result__body">
        <table className="line-items">
          <thead>
            <tr><th>Description</th><th>Miles</th><th>Amount</th></tr>
          </thead>
          <tbody>
            {activeItems.map(([key, item]) => (
              <tr key={key}>
                <td>
                  ...
                  {item.rate != null && <div className="item-rate">{fmtRate(item.rate)}</div>}
                </td>
            ...
            <tr className="line-items__subtotal">
              <td colSpan={2}>Mileage subtotal</td><td>{fmt(quote.subtotalMileage)}</td>
            </tr>
            <tr className="line-items__total">
              <td colSpan={2}>Total Quote</td><td>{fmt(quote.totalQuote)}</td>
            </tr>
          </tbody>
        </table>
