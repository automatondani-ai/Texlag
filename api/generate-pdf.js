 ...
 *
 * Accepts a quote object (same shape returned by /api/quote) plus an optional
 * detentionHourlyRate in the request body, renders a PDF using
 * @react-pdf/renderer on the server, and streams it back as a downloadable
 * application/pdf response.
 *
 * NOTE: This file intentionally uses React.createElement (no JSX) because
 * Vercel serverless functions are not processed by Vite's JSX transform.
 * The layout mirrors /src/components/QuotePDF.jsx exactly.
 */
...
// ── PDF document builder (React.createElement — no JSX required) ─────────────

function buildDocument(quote, detentionHourlyRate) {
  ...
  return h(Document, { title: quote.quoteId, author: 'TexLag Express' },
    h(Page, { size: 'LETTER', style: s.page },
      ...
      h(View, { style: s.body },
        ...
        // Detention compliance — only when detention is NOT charged on this quote
        detentionOff
